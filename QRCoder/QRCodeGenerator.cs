...
public partial class QRCodeGenerator : IDisposable
{
    ...
    private readonly RMQRCodeGenerator _rmqrGenerator;
    ...
    {
        _rmqrGenerator = new RMQRCodeGenerator();
    }
    ...
    public void Dispose() => _rmqrGenerator?.Dispose();

    /// <summary>
    /// Creates an rMQR code with the specified parameters
    /// </summary>
    public QRCodeData CreateRMQRCode(string plainText, RMQRVersion version, RMQRErrorCorrectionLevel eccLevel)
    {
        var rmqrData = _rmqrGenerator.CreateRMQRCode(plainText, version, eccLevel);
        return new QRCodeData(rmqrData);
    }
}
